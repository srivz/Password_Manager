...
class PMPDatabase:
    ...
    def createTable(self):
        qCreate = """CREATE TABLE IF NOT EXISTS masterTable (masterPass varchar(200), email varchar(100))"""
    ...
    def insertIntoTable(self, mp, em):
        ...
        qInsert = """INSERT INTO masterTable (masterPass, email) VALUES (?, ?)"""
    ...
    def updateIntoTable(self, mp):
        ...
        qUpdate = """UPDATE masterTable SET masterPass = ? WHERE email = ?"""
        self.cursor.execute(qUpdate, (hashedMP, mail))
        self.connect.commit()

    def isEmpty(self):
        qCount = """SELECT COUNT(*) FROM masterTable"""
    ...
    def loginCheck(self, mp):
        ...
        qSelect = """SELECT * FROM masterTable"""
        self.cursor.execute(qSelect)
        data = self.cursor.fetchall()

        if data[0][0] == hashedMP:
            return True
        return False
