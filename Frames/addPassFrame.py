...
class AddPassFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.addPassFrame = tk.LabelFrame(self, text="Add New Password", bd=5, bg=self.backgroundColor,
        ...
        self.siteLabel = tk.Label(self.addPassFrame, text='Enter the website name', bg=self.backgroundColor,
                                  fg=self.secTextColor, font=self.labelFont)
        self.siteLabel.place(relx=0.25, rely=0.22, relheight=0.07, relwidth=0.5)
        self.siteText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg=self.surface1Color,
                                 fg=self.secTextColor)
        self.siteText.place(relx=0.25, rely=0.29, relheight=0.05, relwidth=0.5)
        self.usernameLabel = tk.Label(self.addPassFrame, text='Enter the username', bg=self.backgroundColor,
                                      fg=self.secTextColor, font=self.labelFont)
        self.usernameLabel.place(relx=0.25, rely=0.38, relheight=0.07, relwidth=0.5)
        self.usernameText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg=self.surface1Color,
                                     fg=self.secTextColor)
        self.usernameText.place(relx=0.25, rely=0.44, relheight=0.05, relwidth=0.5)
        self.passLabel = tk.Label(self.addPassFrame, text='Enter password', bg=self.backgroundColor,
                                  fg=self.secTextColor, font=self.labelFont)
        self.passLabel.place(relx=0.25, rely=0.52, relheight=0.07, relwidth=0.5)
        self.passText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg=self.surface1Color,
                                 fg=self.secTextColor)
        ...
        self.pwGenBtn = tk.Button(self.addPassFrame, text="Generate Password", command=self.generatePass,
                                  bg=self.surface2Color, fg=self.secTextColor)
        self.pwGenBtn.place(relx=0.175, rely=0.7, relwidth=0.3, relheight=0.07)
        self.saveBtn = tk.Button(self.addPassFrame, text="Save Password", command=self.savePass, bg=self.surface2Color,
                                 fg=self.successColor)
        self.saveBtn.place(relx=0.525, rely=0.7, relwidth=0.3, relheight=0.07)
        self.homeBtn = tk.Button(self.addPassFrame, text="Home",
                                 command=lambda: [self.siteText.delete(0, 'end'), self.usernameText.delete(0, 'end'),
                                                  self.passText.delete(0, 'end'), controller.show_frame(HomeFrame)],
